...
public class GummyLand_Web_Gamble_GambleCount_For_BetType_1_And_Denomination_1 {
	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Gummy Land slot game, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance , spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Gummy_Land_slot_game_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		this.driver = GummyLand_URL_Login.getDriver();
	}

	@When("^Open the Gummy Land slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on Gummy Land slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till user wins, click on gamble button and check the gamble count$")
	public void open_the_Gummy_Land_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_Gummy_Land_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_user_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		// Gamble win amount
		Pattern win_amt1=new Pattern("Images/Gummy_Land/win_g_1_1.PNG");  
		Pattern win_amt2=new Pattern("Images/Gummy_Land/win_g_1_2.PNG");   
		Pattern win_amt3=new Pattern("Images/Gummy_Land/win_g_1_3.PNG");    
		Pattern win_amt4=new Pattern("Images/Gummy_Land/win_g_1_4.PNG");    
		Pattern win_amt5=new Pattern("Images/Gummy_Land/win_g_1_5.PNG");    
		Pattern win_amt6=new Pattern("Images/Gummy_Land/win_g_1_6.PNG");    
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 0.90, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 1.30, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
...
//		Thread.sleep(1000);
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 3.90, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder14.hasNext())
		{
			...
			System.out.println("If the win amount is 6.90, gamble count should be 1.");
			finder14.destroy();  
		}         
		else    
		{ 
		...
		if(finder15.hasNext())
		{
			...
			System.out.println("If the win amount is 20.70, gamble should be disabled");
			finder15.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and gamble max amount configured on the info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Gummy Land game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_gamble_max_amount_configured_on_the_info_page_for_bet_type_denomination_in_Gummy_Land_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
