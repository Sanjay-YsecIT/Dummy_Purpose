...
public class VegasMania_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_3 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Vegas Mania slot game, transfer button, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void chrome_browser_valid_URL_valid_login_details_Vegas_Mania_slot_game_transfer_button_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	    
		this.driver = VegasMania_URL_Login.getDriver();
	}

	@When("^Open the Vegas Mania slot game by entering the valid URL in browser, enter the valid login details, click on Vegas Mania slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_Vegas_Mania_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_Vegas_Mania_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Vegas_Mania/betval3_3.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Vegas_Mania/win_5.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Vegas_Mania/win_10.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Vegas_Mania/win_15.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Vegas_Mania/win_35.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Vegas_Mania/win_75.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Vegas_Mania/win_100.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		// SET THE BET TYPE AS 0.05 AND DENOMIANTION AS 2.5
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 5, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 10, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 15, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 35, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 75, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 87.5 and checking gamble link should be disabled
		...
		if(finder1231.hasNext())
		{
			...
			System.out.println("If the win amount is exceeds 87.5 YSI, gamble link should be disabled.");
			finder1231.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed  on gamble page based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Vegas Mania game$")
	public void gamble_count_should_be_displayed_on_gamble_page_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_in_Vegas_Mania_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
