...
public class TropicanaCasine_Web_PayOut_BetType_4_And_Denomination_All {
	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, Tropicana Casine slot game, icon to open payout table, all symbols, max amount on each symbol like spade, lady n etc, back button, bet type (\\d+)\\.(\\d+) and denomination values like (\\d+), (\\d+), (\\d+), (\\d+) & (\\d+) on web$")
	public void chrome_browser_valid_URL_valid_login_details_Tropicana_Casine_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_spade_lady_n_etc_back_button_bet_type_and_denomination_values_like_on_web(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7) throws Throwable {
	...
	public void open_the_Tropicana_Casine_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_spade_lady_n_etc_click_on_back_button_select_bet_type_as_denomination_values_like_and_check_payout_amount_on_web(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7) throws Throwable {
		...
		Pattern credit4=new Pattern("Images/Tropicana_Casine/credit4.PNG");
		...
		Pattern pat1=new Pattern("Images/Tropicana_Casine/payout4_1.PNG");
		Pattern pat2=new Pattern("Images/Tropicana_Casine/payout4_2.PNG");
		Pattern pat3=new Pattern("Images/Tropicana_Casine/payout4_3.PNG");
		Pattern pat4=new Pattern("Images/Tropicana_Casine/payout4_4.PNG");
		Pattern pat5=new Pattern("Images/Tropicana_Casine/payout4_5.PNG");
	...
	@Then("^System should display the Payout max amount on each symbol like spade, lady n etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination values like (\\d+), (\\d+), (\\d+), (\\d+) & (\\d+) in Tropicana Casine slot game on web$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_spade_lady_n_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_values_like_in_Tropicana_Casine_slot_game_on_web(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
