...
public class TropicanaCasine_Web_Gamble_GambleCount_For_BetType_1_And_Denomination_5 {
	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Tropicana Casine slot game, bet type as (\\d+)\\.(\\d+), denomination as FOUR, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Tropicana_Casine_slot_game_bet_type_as_denomination_as_FOUR_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
		this.driver = TropicanaCasine_URL_Login.getDriver();
	}

	@When("^Open the Tropicana Casine slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on Tropicana Casine slot game, select bet type as (\\d+)\\.(\\d+) & denomination as FOUR, click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Tropicana_Casine_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_Tropicana_Casine_slot_game_select_bet_type_as_denomination_as_FOUR_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
		...
		// Gamble win amount
		Pattern win_amt1=new Pattern("Images/Tropicana_Casine/win_g_5_1.PNG");  
		Pattern win_amt2=new Pattern("Images/Tropicana_Casine/win_g_5_2.PNG");   
		Pattern win_amt3=new Pattern("Images/Tropicana_Casine/win_g_5_3.PNG");    
		Pattern win_amt4=new Pattern("Images/Tropicana_Casine/win_g_5_4.PNG");    
		Pattern win_amt5=new Pattern("Images/Tropicana_Casine/win_g_5_5.PNG");    
		Pattern win_amt6=new Pattern("Images/Tropicana_Casine/win_g_5_6.PNG");    
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 64, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder14.hasNext())
		{
			...
			System.out.println("If the win amount is 128, gamble count should be 1.");
			finder14.destroy();  
		}         
		else    
		{ 
		...
		if(finder15.hasNext())
		{
			...
			System.out.println("If the win amount is 176, gamble should be disabled");
			finder15.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination FOUR in Tropicana Casine game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_FOUR_in_Tropicana_Casine_game(int arg1, int arg2) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
