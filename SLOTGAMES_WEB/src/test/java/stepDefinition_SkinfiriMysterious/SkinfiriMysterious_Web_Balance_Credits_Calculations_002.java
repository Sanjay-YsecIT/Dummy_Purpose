...
public class SkinfiriMysterious_Web_Balance_Credits_Calculations_002 {
	...
	@Given("^Chrome browser, valid URL, valid login details, Skinfiri Mysterious slot game, balance, credits, denomination drop down and denomination as (\\d+)\\.(\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_Skinfiri_Mysterious_slot_game_balance_credits_denomination_drop_down_and_denomination_as(int arg1, int arg2) throws Throwable {
	    
		this.driver =  SkinfiriMysterious_URL_Login.getDriver();
	}

	@When("^Open the Skinfiri Mysterious slot game by entering the valid URL in browser, enter the valid login details, click on balance, select the denomination from drop down, multiply credit by (\\d+)\\.(\\d+) and check the balance$")
	public void open_the_Skinfiri_Mysterious_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_balance_select_the_denomination_from_drop_down_multiply_credit_by_and_check_the_balance(int arg1, int arg2) throws Throwable {
	...
	//	Thread.sleep(15000);
		...
		File DestSrc = new File("./ScreenShots/scrsht/BalToCredit0.02.png");
		...
		// images from the slot games
				...
				Pattern Ysi_bal=new Pattern("./Images/Skinfiri_Mysterious/bal_300.PNG");
				Pattern creditValue=new Pattern("./Images/Skinfiri_Mysterious/0.02_credit.PNG");
				Pattern Credit_bal=new Pattern("./Images/Skinfiri_Mysterious/credit_bal_15000.PNG");
				Pattern new_Ysi_bal=new Pattern("./Images/Skinfiri_Mysterious/bal_300.PNG");
				Pattern credit_incremental=new Pattern("./Images/Skinfiri_Mysterious/credit_incremental.PNG");

				 //String ht = finder.find(pat);s
				// click on incremental credit
				screen.click(credit_incremental);
				  Thread.sleep(3000);
				//Pattern creditC=new Pattern("E:/Sikuli Images/credits/credit1_1.png");
	...
	@Then("^Balance amount should be same as denomination (\\d+)\\.(\\d+) multiplies by credit in Skinfiri Mysterious game$")
	public void balance_amount_should_be_same_as_denomination_multiplies_by_credit_in_Skinfiri_Mysterious_game(int arg1, int arg2) throws Throwable {
