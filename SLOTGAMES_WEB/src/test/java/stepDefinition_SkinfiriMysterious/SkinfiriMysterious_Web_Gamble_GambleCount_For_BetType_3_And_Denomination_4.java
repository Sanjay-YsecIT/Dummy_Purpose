...
public class SkinfiriMysterious_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_4 {
	...
	@Given("^chrome browser, valid URL, valid login details, Skinfiri Mysterious slot game, amount to transfer, transfer button, bet type as (\\d+)\\.(\\d+), denomination as TEN, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Skinfiri_Mysterious_slot_game_amount_to_transfer_transfer_button_bet_type_as_denomination_as_TEN_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
	   
		this.driver = SkinfiriMysterious_URL_Login.getDriver();
	}

	@When("^Open the Skinfiri Mysterious slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Skinfiri Mysterious slot game, transfer the amount, select bet type as (\\d+)\\.(\\d+) & denomination as TEN, click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Skinfiri_Mysterious_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Skinfiri_Mysterious_slot_game_transfer_the_amount_select_bet_type_as_denomination_as_TEN_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
	...
	//	Pattern BALANCE=new Pattern("./Images/Skinfiri_Mysterious/bal_300.PNG");                   // BAL AT INITIAL STAGE
		...
		Pattern DENO_10=new Pattern("./Images/Skinfiri_Mysterious/betvalue3_4.PNG");                      // DENOMINATION OF 10
	...
	//	Pattern CREDIT = new Pattern("./Images/Skinfiri_Mysterious/0.02_credit.PNG");
		Pattern CREDIT_INC = new Pattern("./Images/Skinfiri_Mysterious/credit_incremental.PNG");   // INCREMENTAL FROM 0.01 TO 0.05
		...
		Pattern WIN_AMT1=new Pattern("./Images/Skinfiri_Mysterious/05_win1_17.5.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Skinfiri_Mysterious/05_win2_30.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Skinfiri_Mysterious/05_win3_50.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Skinfiri_Mysterious/05_win4_100.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Skinfiri_Mysterious/05_win5_250.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Skinfiri_Mysterious/05_win_1200.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 17.5, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 50, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 250, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and max amount of the gamble configured on the game info page for bet type (\\d+)\\.(\\d+) and denomination TEN in Skinfiri Mysterious game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_max_amount_of_the_gamble_configured_on_the_game_info_page_for_bet_type_and_denomination_TEN_in_Skinfiri_Mysterious_game(int arg1, int arg2) throws Throwable {
