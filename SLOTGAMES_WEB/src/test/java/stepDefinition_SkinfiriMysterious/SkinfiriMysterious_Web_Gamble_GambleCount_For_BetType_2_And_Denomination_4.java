...
public class SkinfiriMysterious_Web_Gamble_GambleCount_For_BetType_2_And_Denomination_4 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Skinfiri Mysterious slot game, amount to transfer, transfer button, bet type as (\\d+)\\.(\\d+), denomination as FOUR, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Skinfiri_Mysterious_slot_game_amount_to_transfer_transfer_button_bet_type_as_denomination_as_FOUR_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
	   
		this.driver = SkinfiriMysterious_URL_Login.getDriver();
	}

	@When("^Open the Skinfiri Mysterious slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Skinfiri Mysterious slot game, transfer the amount, select bet type as (\\d+)\\.(\\d+) & denomination as FOUR, click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Skinfiri_Mysterious_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Skinfiri_Mysterious_slot_game_transfer_the_amount_select_bet_type_as_denomination_as_FOUR_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
	...
	//	Pattern BALANCE=new Pattern("./Images/Skinfiri_Mysterious/bal_300.PNG");                   // BAL AT INITIAL STAGE
		...
		Pattern DENO_4=new Pattern("./Images/Skinfiri_Mysterious/betvalue2_4.PNG");                      // DENOMINATION OF 0.01
	...
	//	Pattern CREDIT = new Pattern("./Images/Skinfiri_Mysterious/0.02_credit.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Skinfiri_Mysterious/02_win1_7.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Skinfiri_Mysterious/02_win2_12.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Skinfiri_Mysterious/02_win3_20.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Skinfiri_Mysterious/02_win4_40.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Skinfiri_Mysterious/02_win5_100.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Skinfiri_Mysterious/02_win_480.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		//Arranging the symbols to generate gamble count as 5
				...
				if(finder1.hasNext())
				{
					...
					System.out.println("If the win amount is 7, gamble count should be 5.");
					finder1.destroy();  
				}         
				else    
				{ 
				...
				if(finder12.hasNext())
				{
					...
					System.out.println("If the win amount is 20, gamble count should be 3.");
					finder12.destroy();  
				}         
				else    
				{ 
				...
				if(finder123.hasNext())
				{
					...
					System.out.println("If the win amount is 100, gamble count should be 1.");
					finder123.destroy();  
				}         
				else    
				{ 
	...
	@Then("^Gamble count should be displayed based on win amount and max amount of the gamble configured on the game info page for bet type (\\d+)\\.(\\d+) and denomination FOUR in Skinfiri Mysterious game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_max_amount_of_the_gamble_configured_on_the_game_info_page_for_bet_type_and_denomination_FOUR_in_Skinfiri_Mysterious_game(int arg1, int arg2) throws Throwable {
