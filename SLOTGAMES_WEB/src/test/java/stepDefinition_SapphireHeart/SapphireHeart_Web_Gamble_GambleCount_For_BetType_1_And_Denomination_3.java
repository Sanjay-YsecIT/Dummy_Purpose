...
public class SapphireHeart_Web_Gamble_GambleCount_For_BetType_1_And_Denomination_3 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Sapphire Heart slot game, bet type as (\\d+)\\.(\\d+), denomination as THREE, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void sapphire_Heart_slot_game_bet_type_as_denomination_as_THREE_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2) throws Throwable {
	    
		this.driver = SapphireHeart_URL_Login.getDriver();
	}

	@When("^Open the Sapphire Heart slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Sapphire Heart slot game, select bet type as (\\d+)\\.(\\d+) & denomination as THREE, click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_Sapphire_Heart_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Sapphire_Heart_slot_game_select_bet_type_as_denomination_as_THREE_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Sapphire_Heart/betval1_3.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Sapphire_Heart/win_5.4.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Sapphire_Heart/win_6.9.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Sapphire_Heart/win_21.75.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Sapphire_Heart/win_41.7.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Sapphire_Heart/win_55.2.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Sapphire_Heart/win_111.45.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		// SET THE BET TYPE AS 0.01 AND DENOMIANTION AS 3
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 1.80, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 2.30, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 7.25, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 13.9, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 18.4, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 105 and checking gamble link should be disabled
		...
		if(finder1231.hasNext())
		{
			...
			System.out.println("If the win amount is exceeds 105 YSI, gamble link should be disabled.");
			finder1231.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed  on gamble page of Sapphire Heart slot game based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination THREE in Sapphire Heart game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_Sapphire_Heart_slot_game_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_THREE_in_Sapphire_Heart_game(int arg1, int arg2) throws Throwable {
