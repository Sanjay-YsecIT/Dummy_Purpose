...
public class SapphireHeart_Web_Gamble_GambleCount_For_BetType_2_And_Denomination_4 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Sapphire Heart slot game, amount to transfer, transfer button, bet type as (\\d+)\\.(\\d+) denomination as SIXTEEN, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Sapphire_Heart_slot_game_amount_to_transfer_transfer_button_bet_type_as_denomination_as_SIXTEEN_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
	    
		this.driver = SapphireHeart_URL_Login.getDriver();
	}

	@When("^Open the Sapphire Heart slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Sapphire Heart slot game, transfer the amount, select bet type as (\\d+)\\.(\\d+) & denomination as SIXTEEN, click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Sapphire_Heart_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Sapphire_Heart_slot_game_transfer_the_amount_select_bet_type_as_denomination_as_SIXTEEN_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Sapphire_Heart/betval2_4.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Sapphire_Heart/win_28.8.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Sapphire_Heart/win_36.8.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Sapphire_Heart/win_116.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Sapphire_Heart/win_222.4.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Sapphire_Heart/win_297.4.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Sapphire_Heart/win_597.4.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		// SET THE BET TYPE AS 0.04 AND DENOMIANTION AS 16
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 28.80, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 36.80, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 116, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 222.40, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 294.40, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 560 and checking gamble link should be disabled
		...
		if(finder1231.hasNext())
		{
			...
			System.out.println("If the win amount is exceeds 560 YSI, gamble link should be disabled.");
			finder1231.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and max amount of the gamble configured on the game info page for bet type (\\d+)\\.(\\d+) and denomination SIXTEEN in Sapphire Heart game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_max_amount_of_the_gamble_configured_on_the_game_info_page_for_bet_type_and_denomination_SIXTEEN_in_Sapphire_Heart_game(int arg1, int arg2) throws Throwable {
