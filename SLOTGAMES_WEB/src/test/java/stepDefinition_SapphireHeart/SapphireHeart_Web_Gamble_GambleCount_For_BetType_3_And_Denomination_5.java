...
public class SapphireHeart_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_5 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^chrome browser, valid URL, valid login details, Sapphire Heart slot game, bet type as (\\d+)\\.(\\d+), denomination as THIRTY, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Sapphire_Heart_slot_game_bet_type_as_denomination_as_THIRTY_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
	    
		this.driver = SapphireHeart_URL_Login.getDriver();
	}

	@When("^Open the Sapphire Heart slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on Sapphire Heart slot game, select bet type as (\\d+)\\.(\\d+) & denomination as THIRTY click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Sapphire_Heart_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_Sapphire_Heart_slot_game_select_bet_type_as_denomination_as_THIRTY_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Sapphire_Heart/betval3_5.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Sapphire_Heart/win_54.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Sapphire_Heart/win_69.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Sapphire_Heart/win_217.5.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Sapphire_Heart/win_417.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Sapphire_Heart/win_552.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Sapphire_Heart/win_1114.5.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		// SET THE BET TYPE AS 0.06 AND DENOMIANTION AS 30
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 54, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 69, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 217.50, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 417, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 552, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 1050 and checking gamble link should be disabled
		...
		if(finder1231.hasNext())
		{
			...
			System.out.println("If the win amount is exceeds 1050 YSI, gamble link should be disabled.");
			finder1231.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination THIRTY in Sapphire Heart game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_THIRTY_in_Sapphire_Heart_game(int arg1, int arg2) throws Throwable {
