...
public class LadyCharm_Web_Gamble_GambleCount_For_BetType_2_And_Denomination_2 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Lady Charm slot game, transfer button, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page in gamble page of slot game$")
	public void chrome_browser_valid_URL_valid_login_details_Lady_Charm_slot_game_transfer_button_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	    
		this.driver = LadyCharm_URL_Login.getDriver();
	}

	@When("^Open the Lady Charm slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Lady Charm slot game, transfer amount to slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count in gamble page$")
	public void open_the_Lady_Charm_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Lady_Charm_slot_game_transfer_amount_to_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		Pattern SPIN=new Pattern("./Images/Lady_Charm/spin.PNG");                         // SPIN BUTTON
		Pattern GAMBLE_COLLECT=new Pattern("./Images/Lady_Charm/gb_collect.PNG");  // AMOUNT COLLECT ICON ON GAMBLE WINDOW          
		Pattern GAMBLE_BTN=new Pattern("./Images/Lady_Charm/gb_button.PNG");     // GAMBLE BUTTON TO SWITCH TO GAMBLE WINDOW
		Pattern winBF=new Pattern("./Images/Lady_Charm/gb_disable.PNG");         // GAMBLE DISABLE BUTTON 
		Pattern betval=new Pattern("./Images/Lady_Charm/betval2_2.PNG");
		...
		Pattern wild3=new Pattern("./Images/Lady_Charm/3R_wild.PNG");
		Pattern ten3=new Pattern("./Images/Lady_Charm/3R_10.PNG");
		Pattern HorseShoe3=new Pattern("./Images/Lady_Charm/3R_shoe.PNG");
		Pattern nine3=new Pattern("./Images/Lady_Charm/3R_9.PNG");

		// IMAGES IN 4th REELS
		Pattern ten4=new Pattern("./Images/Lady_Charm/4R_10.PNG");
		Pattern HorseShoe4=new Pattern("./Images/Lady_Charm/4R_shoe.PNG");
		Pattern A4=new Pattern("./Images/Lady_Charm/4R_a.PNG");
		Pattern wild4=new Pattern("./Images/Lady_Charm/4R_wild.PNG");
		Pattern leaf4=new Pattern("./Images/Lady_Charm/4R_leaf.PNG");
		Pattern HorseShoe41=new Pattern("./Images/Lady_Charm/4R_shoe1.PNG");
		Pattern nine4=new Pattern("./Images/Lady_Charm/4R_9.PNG");

		// IMAGES IN 5th REELS
		Pattern K5=new Pattern("./Images/Lady_Charm/5R_k.PNG");


		Pattern WIN_AMT1=new Pattern("./Images/Lady_Charm/win_0.64.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Lady_Charm/win_3.2.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Lady_Charm/win_4.8.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Lady_Charm/win_8.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Lady_Charm/win_20.6.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Lady_Charm/win_32.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0


		// ATTEMPTS LEFT
		Pattern Attempt_5=new Pattern("./Images/Lady_Charm/Attempt5.PNG");
		Pattern Attempt_4=new Pattern("./Images/Lady_Charm/Attempt4.PNG");
		Pattern Attempt_3=new Pattern("./Images/Lady_Charm/Attempt3.PNG");
		Pattern Attempt_2=new Pattern("./Images/Lady_Charm/Attempt2.PNG");
		Pattern Attempt_1=new Pattern("./Images/Lady_Charm/Attempt1.PNG");


		// SET THE BET TYPE AS 0.02 AND DENOMIANTION AS 0.8
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 0.64, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 3.20, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 4.80, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 8, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 20.6, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 28 and checking gamble link should be disabled

		screen.click(HorseShoe41);
		Thread.sleep(1000);
		screen.click(SPIN);
		Thread.sleep(6000);
		...
		if(finder1231.hasNext())
		{
			...
			System.out.println("If the win amount is exceeds 28 YSI, gamble link should be disabled.");
			finder1231.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed on gamble page of Lady Charm slot game based on winning amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Lady Charm game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_Lady_Charm_slot_game_based_on_winning_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_in_Lady_Charm_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
