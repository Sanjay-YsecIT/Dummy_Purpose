...
public class AmazonClash_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_3 {

	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, Amazon Clash slot game, transfer button, bet type as (\\d+)\\.(\\d+), denomination as FIVE, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void chrome_browser_valid_URL_valid_login_details_Amazon_Clash_slot_game_transfer_button_bet_type_as_denomination_as_FIVE_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2) throws Throwable {

		this.driver = AmazonClash_URL_Login.getDriver();
	}

	@When("^Open the Amazon Clash slot game by entering the valid URL in browser, enter the valid login details, click on Amazon Clash slot game, select bet type as (\\d+)\\.(\\d+) & denomination as FIVE, click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_Amazon_Clash_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_Amazon_Clash_slot_game_select_bet_type_as_denomination_as_FIVE_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Amazon_Clash/betval3_3.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Amazon_Clash/win_10.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Amazon_Clash/win_15.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Amazon_Clash/win_32.5.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Amazon_Clash/win_45.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Amazon_Clash/win_102.5.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Amazon_Clash/win_175.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 32.5, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 45, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed  on gamble page based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination FIVE in Amazon Clash game$")
	public void gamble_count_should_be_displayed_on_gamble_page_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_FIVE_in_Amazon_Clash_game(int arg1, int arg2) throws Throwable {
