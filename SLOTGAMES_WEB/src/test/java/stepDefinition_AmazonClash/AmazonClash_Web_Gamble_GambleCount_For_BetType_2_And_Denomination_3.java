...
public class AmazonClash_Web_Gamble_GambleCount_For_BetType_2_And_Denomination_3 {

	WebDriver driver;
	Screen screen=new Screen();
	
	
	@Given("^Chrome browser, Valid URL, valid Login credentials, Amazon Clash slot game, bet type as (\\d+)\\.(\\d+), denomination as TWO, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void chrome_browser_Valid_URL_valid_Login_credentials_Amazon_Clash_slot_game_bet_type_as_denomination_as_TWO_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2) throws Throwable {
	   
		this.driver = AmazonClash_URL_Login.getDriver();
	}

	@When("^Open the Amazon Clash slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Amazon Clash slot game, select bet type as (\\d+)\\.(\\d+) & denomination as TWO, click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_Amazon_Clash_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Amazon_Clash_slot_game_select_bet_type_as_denomination_as_TWO_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Amazon_Clash/betval2_3.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Amazon_Clash/win_4.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Amazon_Clash/win_6.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Amazon_Clash/win_13.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Amazon_Clash/win_18.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Amazon_Clash/win_41.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Amazon_Clash/win_70.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 41, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed  on gamble page of Amazon Clash slot game based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination TWO in Amazon Clash game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_Amazon_Clash_slot_game_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_TWO_in_Amazon_Clash_game(int arg1, int arg2) throws Throwable {
