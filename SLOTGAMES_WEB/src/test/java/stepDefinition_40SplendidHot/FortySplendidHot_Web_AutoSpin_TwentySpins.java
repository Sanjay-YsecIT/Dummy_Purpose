...
public class FortySplendidHot_Web_AutoSpin_TwentySpins {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, FortySplendidHot slot game, balance, spin button, auto spin button, twenty spins option and number of spins left message$")
	public void chrome_browser_valid_URL_valid_login_details_FortySplendidHot_slot_game_balance_spin_button_auto_spin_button_twenty_spins_option_and_number_of_spins_left_message() throws Throwable {
	...
	@When("^Open the FortySplendidHot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on twenty spin option under auto spin drop down and check the number of spins left message$")
	public void open_the_FortySplendidHot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_twenty_spin_option_under_auto_spin_drop_down_and_check_the_number_of_spins_left_message() throws Throwable {
		...
		Pattern autospin_20=new Pattern("Images/40Splendid_Hot/20spin.PNG");  

		Pattern pat2=new Pattern("Images/40Splendid_Hot/19spin_left.PNG"); 
		Pattern pat3=new Pattern("Images/40Splendid_Hot/17spin_left.PNG"); 
		Pattern pat4=new Pattern("Images/40Splendid_Hot/15spin_left.PNG"); 
		Pattern pat5=new Pattern("Images/40Splendid_Hot/13spin_left.PNG"); 
		Pattern pat6=new Pattern("Images/40Splendid_Hot/11spin_left.PNG"); 
		Pattern pat7=new Pattern("Images/40Splendid_Hot/9spin_left.PNG");  
		Pattern pat8=new Pattern("Images/40Splendid_Hot/7spin_left.PNG");  
		Pattern pat10=new Pattern("Images/40Splendid_Hot/2spin_left.PNG"); 
		...
		Assert.assertTrue(score9 > 85);
	}

	@Then("^Number of spin left should start from (\\d+) to (\\d+) after clicking twenty spins option and should perform (\\d+) spins from (\\d+) to (\\d+) in FortySplendidHot slot game$")
	public void number_of_spin_left_should_start_from_to_after_clicking_twenty_spins_option_and_should_perform_spins_from_to_in_FortySplendidHot_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
