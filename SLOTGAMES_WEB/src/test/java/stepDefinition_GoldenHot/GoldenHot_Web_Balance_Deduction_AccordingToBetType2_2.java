...
public class GoldenHot_Web_Balance_Deduction_AccordingToBetType2_2 {
	...
	@When("^Open the Golden Hot slot game by entering the valid URL in browser, enter the valid login details, select the bet type as (\\d+)\\.(\\d+) from drop down, bet value to (\\d+)\\.(\\d+) click on spin button and check the balance$")
	public void open_the_Golden_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_from_drop_down_bet_value_to_click_on_spin_button_and_check_the_balance(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	...
	@Then("^Balance should get deducted by (\\d+)\\.(\\d+) if the bet type is selected as (\\d+)\\.(\\d+) in Golden Hot hot game$")
	public void balance_should_get_deducted_by_if_the_bet_type_is_selected_as_in_Golden_Hot_hot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
