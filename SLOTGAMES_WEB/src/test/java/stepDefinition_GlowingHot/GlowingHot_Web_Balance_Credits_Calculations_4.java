...
public class GlowingHot_Web_Balance_Credits_Calculations_4 {

	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, Glowing Hot slot game, balance, credits and denomination drop down & value as (\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_Glowing_Hot_slot_game_balance_credits_and_denomination_drop_down_value_as(int arg1) throws Throwable {

		this.driver = GlowingHot_URL_Login.getDriver();
	}

	@When("^Open the Glowing Hot slot game by entering the valid URL in browser, enter the valid login details, click on balance, multiply credit by denomination (\\d+) manually with and check the balance$")
	public void open_the_Glowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_balance_multiply_credit_by_denomination_manually_with_and_check_the_balance(int arg1) throws Throwable {
		...
		Pattern Ysi_bal=new Pattern("./Images/Glowing_Hot/bal_300.PNG");
		Pattern creditValue=new Pattern("./Images/Glowing_Hot/credit_1.PNG");
		Pattern Credit_bal=new Pattern("./Images/Glowing_Hot/cr_bal_300.PNG");
		Pattern new_Ysi_bal=new Pattern("./Images/Glowing_Hot/bal_300.PNG");
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("Credit value comparision successful. Its 1");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder2.hasNext())
		{
			...
			System.out.println("Balance in credits is 300");
			finder2.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Balance amount should be same after multiplying denomination (\\d+) by credit in Glowing Hot slot game$")
	public void balance_amount_should_be_same_after_multiplying_denomination_by_credit_in_Glowing_Hot_slot_game(int arg1) throws Throwable {
