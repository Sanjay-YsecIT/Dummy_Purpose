...
public class GlowingHot_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_2 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Glowing Hot slot game, transfer button, bet type as (\\d+)\\.(\\d+), denomination as ONE, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page$")
	public void chrome_browser_valid_URL_valid_login_details_Glowing_Hot_slot_game_transfer_button_bet_type_as_denomination_as_ONE_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page(int arg1, int arg2) throws Throwable {
	    
		this.driver = GlowingHot_URL_Login.getDriver();
	}

	@When("^Open the Glowing Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on Glowing Hot slot game, transfer amount to slot game, select bet type as (\\d+)\\.(\\d+) & denomination as ONE, click on spin button till player wins, click on gamble button and check the gamble count in gamble page$")
	public void open_the_Glowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_Glowing_Hot_slot_game_transfer_amount_to_slot_game_select_bet_type_as_denomination_as_ONE_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Glowing_Hot/betval3_2.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/Glowing_Hot/win_2.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Glowing_Hot/win_4.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Glowing_Hot/win_6.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Glowing_Hot/win_12.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Glowing_Hot/win_26.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Glowing_Hot/win_66.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 12, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 26, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on winning amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination ONE in Glowing Hot game$")
	public void gamble_count_should_be_displayed_based_on_winning_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_ONE_in_Glowing_Hot_game(int arg1, int arg2) throws Throwable {
