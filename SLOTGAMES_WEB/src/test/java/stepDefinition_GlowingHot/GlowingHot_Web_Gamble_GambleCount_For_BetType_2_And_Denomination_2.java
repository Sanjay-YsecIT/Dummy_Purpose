...
public class GlowingHot_Web_Gamble_GambleCount_For_BetType_2_And_Denomination_2 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Chrome browser, valid URL, valid login details, Glowing Hot slot game, transfer button, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page in gamble page of slot game$")
	public void chrome_browser_valid_URL_valid_login_details_Glowing_Hot_slot_game_transfer_button_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	    
		this.driver = GlowingHot_URL_Login.getDriver();
	}

	@When("^Open the Glowing Hot slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Glowing Hot slot game, transfer amount to slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count in gamble page$")
	public void open_the_Glowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Glowing_Hot_slot_game_transfer_amount_to_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		Pattern SPIN=new Pattern("./Images/Glowing_Hot/spin.PNG");                         // SPIN BUTTON
		Pattern GAMBLE_COLLECT=new Pattern("./Images/Glowing_Hot/gb_collect.PNG");  // AMOUNT COLLECT ICON ON GAMBLE WINDOW          
		Pattern GAMBLE_BTN=new Pattern("./Images/Glowing_Hot/gb_button.PNG");     // GAMBLE BUTTON TO SWITCH TO GAMBLE WINDOW
		Pattern winBF=new Pattern("./Images/Glowing_Hot/gb_disable.PNG");         // GAMBLE DISABLE BUTTON 
		Pattern betval=new Pattern("./Images/Glowing_Hot/betval2_2.PNG");
		...
		Pattern lemon21=new Pattern("./Images/Glowing_Hot/2R_lemon1.PNG");
		Pattern berry2=new Pattern("./Images/Glowing_Hot/2R_berry.PNG");
		Pattern seven2=new Pattern("./Images/Glowing_Hot/2R_seven.PNG");

		// IMAGES IN 3rd REELS
		Pattern berry3=new Pattern("./Images/Glowing_Hot/3R_berry.PNG");
		Pattern orange3=new Pattern("./Images/Glowing_Hot/3R_orange.PNG");

		// IMAGES IN 4th REELS
		Pattern lemon4=new Pattern("./Images/Glowing_Hot/4R_lemon.PNG");

		// IMAGES IN 5th REELS
		Pattern seven5=new Pattern("./Images/Glowing_Hot/5R_seven.PNG");

		Pattern WIN_AMT1=new Pattern("./Images/Glowing_Hot/win_1.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Glowing_Hot/win_2.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Glowing_Hot/win_3.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Glowing_Hot/win_6.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Glowing_Hot/win_13.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Glowing_Hot/win_33.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0

		// ATTEMPTS LEFT
		Pattern Attempt_5=new Pattern("./Images/Glowing_Hot/Attempt5.PNG");
		Pattern Attempt_4=new Pattern("./Images/Glowing_Hot/Attempt4.PNG");
		Pattern Attempt_3=new Pattern("./Images/Glowing_Hot/Attempt3.PNG");
		Pattern Attempt_2=new Pattern("./Images/Glowing_Hot/Attempt2.PNG");
		Pattern Attempt_1=new Pattern("./Images/Glowing_Hot/Attempt1.PNG");
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 6, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 13, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 5 and checking gamble link should be disabled
		screen.click(lemon4);
		Thread.sleep(1000);
		screen.click(SPIN);
		Thread.sleep(6000);
	...
	@Then("^Gamble count should be displayed on gamble page of Glowing Hot slot game based on winning amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Glowing Hot game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_Glowing_Hot_slot_game_based_on_winning_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_in_Glowing_Hot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
