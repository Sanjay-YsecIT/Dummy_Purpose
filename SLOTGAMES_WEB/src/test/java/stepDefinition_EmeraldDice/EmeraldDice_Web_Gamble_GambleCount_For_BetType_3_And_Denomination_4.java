...
public class EmeraldDice_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_4 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^chrome browser, valid URL, valid login details, Emerald Dice slot game, amount to transfer, transfer button, bet type as (\\d+)\\.(\\d+), denomination as TEN, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Emerald_Dice_slot_game_amount_to_transfer_transfer_button_bet_type_as_denomination_as_TEN_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
	   
		this.driver = EmeraldDice_URL_Login.getDriver();
	}

	@When("^Open the Emerald Dice slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Emerald Dice slot game, transfer the amount, select bet type as (\\d+)\\.(\\d+) & denomination as TEN, click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Emerald_Dice_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Emerald_Dice_slot_game_transfer_the_amount_select_bet_type_as_denomination_as_TEN_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/Emerald_Dice/betval3_4.PNG");          // betvalue 10
		...
		// win3_54 represents win amt for 0.04 bettype and 5,4,3,2,1 and 0 represents attempts, 1 represents betvalue of 10 

		Pattern WIN_AMT1=new Pattern("./Images/Emerald_Dice/win3_54.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/Emerald_Dice/win3_44.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/Emerald_Dice/win3_34.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/Emerald_Dice/win3_24.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/Emerald_Dice/win3_14.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/Emerald_Dice/win3_04.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		// SET THE BET TYPE AS 0.02 AND DENOMIANTION AS 0.05
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 27.50, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 55, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 290, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and max amount of the gamble configured on the game info page for bet type (\\d+)\\.(\\d+) and denomination TEN in Emerald Dice game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_max_amount_of_the_gamble_configured_on_the_game_info_page_for_bet_type_and_denomination_TEN_in_Emerald_Dice_game(int arg1, int arg2) throws Throwable {
