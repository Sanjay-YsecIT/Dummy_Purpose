...
public class FortyGlowingHot_Web_Balance_Credits_Calculations_4 {

	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, FortyGlowing Hot slot game, balance, credits and denomination drop down & value as (\\d+)\\.(\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_FortyGlowing_Hot_slot_game_balance_credits_and_denomination_drop_down_value_as(int arg1, int arg2) throws Throwable {

		this.driver = FortyGlowingHot_URL_Login.getDriver();
	}

	@When("^Open the FortyGlowing Hot slot game by entering the valid URL in browser, enter the valid login details, click on balance, multiply credit by denomination (\\d+)\\.(\\d+) manually with and check the balance$")
	public void open_the_FortyGlowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_balance_multiply_credit_by_denomination_manually_with_and_check_the_balance(int arg1, int arg2) throws Throwable {
		...
		Pattern Ysi_bal=new Pattern("./Images/20Glowing_Hot/bal_300.PNG");
		Pattern creditValue=new Pattern("./Images/20Glowing_Hot/credit_0.2.PNG");
		Pattern Credit_bal=new Pattern("./Images/20Glowing_Hot/cr_bal_1500.PNG");
		Pattern new_Ysi_bal=new Pattern("./Images/20Glowing_Hot/bal_300.PNG");
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("Credit value comparision successful. Its 0.3");
			finder1.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Balance amount should be same after multiplying denomination (\\d+)\\.(\\d+) by credit in FortyGlowing Hot slot game$")
	public void balance_amount_should_be_same_after_multiplying_denomination_by_credit_in_FortyGlowing_Hot_slot_game(int arg1, int arg2) throws Throwable {
