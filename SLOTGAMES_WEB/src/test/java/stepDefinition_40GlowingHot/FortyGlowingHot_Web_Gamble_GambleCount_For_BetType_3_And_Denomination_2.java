...
public class FortyGlowingHot_Web_Gamble_GambleCount_For_BetType_3_And_Denomination_2 {

	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^chrome Browser, valid URL, valid login details, FortyGlowing Hot game, transfer amount button, bet type as (\\d+)\\.(\\d+), betvalue as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, gamble to win amount game info page and gamble count in gamble page$")
	public void chrome_Browser_valid_URL_valid_login_details_FortyGlowing_Hot_game_transfer_amount_button_bet_type_as_betvalue_as_balance_spin_button_win_amount_gamble_button_gamble_amount_gamble_to_win_amount_game_info_page_and_gamble_count_in_gamble_page(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	   
		this.driver = FortyGlowingHot_URL_Login.getDriver();
	}

	@When("^open the fortyGlowing Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on FortyGlowing Hot game, transfer amount to slot game, select bet type as (\\d+)\\.(\\d+) & betvalue as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_fortyGlowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_FortyGlowing_Hot_game_transfer_amount_to_slot_game_select_bet_type_as_betvalue_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/40Glowing_Hot/betval3_2.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/40Glowing_Hot/win_2.4.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/40Glowing_Hot/win_6.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/40Glowing_Hot/win_12.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/40Glowing_Hot/win_24.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/40Glowing_Hot/win_48.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/40Glowing_Hot/win_84.6.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		// SET THE BET TYPE AS 0.03 AND DENOMIANTION AS 2.4
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 2.4, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 24, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder123.hasNext())
		{
			...
			System.out.println("If the win amount is 48, gamble count should be 1.");
			finder123.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount more than 84 and checking gamble link should be disabled
		screen.click(bell4);
		Thread.sleep(1000);
		screen.click(SPIN);
		Thread.sleep(6000);
		...
		if(finder1231.hasNext())
		{
			...
			System.out.println("If the win amount is exceeds 84 YSI, gamble link should be disabled.");
			finder1231.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble Count ranging from (\\d+)-(\\d+) should be displayed based on winning amount and gamble max amount configured on the game info page for credit (\\d+)\\.(\\d+) & betvalue (\\d+)\\.(\\d+) in FortyGlowing Hot slot game$")
	public void gamble_Count_ranging_from_should_be_displayed_based_on_winning_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_credit_betvalue_in_FortyGlowing_Hot_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6) throws Throwable {
