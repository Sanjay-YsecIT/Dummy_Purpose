...
public class TwentyGlowingHot_Web_Gamble_GambleCount_For_BetType_2_And_Denomination_1 {

	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, TwentyGlowing Hot slot game, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void chrome_browser_valid_URL_valid_login_details_TwentyGlowing_Hot_slot_game_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {

		this.driver = TwentyGlowingHot_URL_Login.getDriver();
	}

	@When("^Open the TwentyGlowing Hot slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on TwentyGlowing Hot slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till user wins, click on gamble button and check the gamble count$")
	public void open_the_TwentyGlowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_TwentyGlowing_Hot_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_user_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		Pattern betval=new Pattern("./Images/20Glowing_Hot/betval2_1.PNG");
		...
		Pattern WIN_AMT1=new Pattern("./Images/20Glowing_Hot/win_0.4.PNG");    // 1st WIN AMOUNT WITH GAMBLE ATTEMPTS 5
		Pattern WIN_AMT2=new Pattern("./Images/20Glowing_Hot/win_1.PNG");    // 2st WIN AMOUNT WITH GAMBLE ATTEMPTS 4
		Pattern WIN_AMT3=new Pattern("./Images/20Glowing_Hot/win_2.8.PNG");    // 3st WIN AMOUNT WITH GAMBLE ATTEMPTS 3
		Pattern WIN_AMT4=new Pattern("./Images/20Glowing_Hot/win_4.PNG");    // 4st WIN AMOUNT WITH GAMBLE ATTEMPTS 2
		Pattern WIN_AMT5=new Pattern("./Images/20Glowing_Hot/win_10.PNG");    // 5st WIN AMOUNT WITH GAMBLE ATTEMPTS 1
		Pattern WIN_AMT6=new Pattern("./Images/20Glowing_Hot/win_17.PNG");    // 6st WIN AMOUNT WITH GAMBLE ATTEMPTS 0
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 2.80, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed on gamble page of TwentyGlowing Hot slot game based on win amount and gamble max amount configured on the info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in TwentyGlowing Hot game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_TwentyGlowing_Hot_slot_game_based_on_win_amount_and_gamble_max_amount_configured_on_the_info_page_for_bet_type_denomination_in_TwentyGlowing_Hot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
