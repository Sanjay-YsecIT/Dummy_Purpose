...
public class BrownieHot_Web_Gamble_GambleCount_For_BetType_1_And_Denomination_4 {

	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, Brownie Hot slot game, amount to transfer, transfer button, bet type as (\\d+)\\.(\\d+), denomination as Three, balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count$")
	public void chrome_browser_valid_URL_valid_login_details_Brownie_Hot_slot_game_amount_to_transfer_transfer_button_bet_type_as_denomination_as_Three_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count(int arg1, int arg2) throws Throwable {
		this.driver = BrownieHot_URL_Login.getDriver();
	}

	@When("^Open the Brownie Hot slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Brownie Hot slot game, transfer the amount, select bet type as (\\d+)\\.(\\d+) & denomination as Three, click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Brownie_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Brownie_Hot_slot_game_transfer_the_amount_select_bet_type_as_denomination_as_Three_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2) throws Throwable {
		...
		// Gamble win amount
		Pattern win_amt1=new Pattern("Images/Brownie_Hot/win_g_1_4.PNG");  
		Pattern win_amt2=new Pattern("Images/Brownie_Hot/win_g_2_4.PNG");   
		Pattern win_amt3=new Pattern("Images/Brownie_Hot/win_g_3_4.PNG");    
		Pattern win_amt4=new Pattern("Images/Brownie_Hot/win_g_4_4.PNG");    
		Pattern win_amt5=new Pattern("Images/Brownie_Hot/win_g_5_4.PNG");    
		Pattern win_amt6=new Pattern("Images/Brownie_Hot/win_g_6_4.PNG");    
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 7, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 17.5, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 47.5, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder14.hasNext())
		{
			...
			System.out.println("If the win amount is 102.5, gamble count should be 1.");
			finder14.destroy();  
		}         
		else    
		{ 
		...
		if(finder15.hasNext())
		{
			...
			System.out.println("If the win amount is 120, gamble should be disabled");
			finder15.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed based on win amount and max amount of the gamble configured on the game info page for bet type (\\d+)\\.(\\d+) and denomination Three in Brownie Hot game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_max_amount_of_the_gamble_configured_on_the_game_info_page_for_bet_type_and_denomination_Three_in_Brownie_Hot_game(int arg1, int arg2) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
