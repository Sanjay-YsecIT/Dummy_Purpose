...
public class BrownieHot_Web_Gamble_GambleCount_For_BetType_1_And_Denomination_3 {

	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Brownie Hot slot game, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void brownie_Hot_slot_game_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		this.driver = BrownieHot_URL_Login.getDriver();
	}

	@When("^Open the Brownie Hot slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Brownie Hot slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_Brownie_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Brownie_Hot_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		// Gamble win amount
		Pattern win_amt1=new Pattern("Images/Brownie_Hot/win_g_1_3.PNG");  
		Pattern win_amt2=new Pattern("Images/Brownie_Hot/win_g_2_3.PNG");   
		Pattern win_amt3=new Pattern("Images/Brownie_Hot/win_g_3_3.PNG");    
		Pattern win_amt4=new Pattern("Images/Brownie_Hot/win_g_4_3.PNG");    
		Pattern win_amt5=new Pattern("Images/Brownie_Hot/win_g_5_3.PNG");    
		Pattern win_amt6=new Pattern("Images/Brownie_Hot/win_g_6_3.PNG");    
		...
		// SET THE BET TYPE AS 0.01 AND DENOMIANTION AS 1.5
		...
		if(finder11.hasNext())
		{
			...
			System.out.println("If the win amount is 3.5, gamble count should be 4.");
			finder11.destroy();  
		}         
		else    
		{ 
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("If the win amount is 8.75, gamble count should be 3.");
			finder12.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 23.75, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder14.hasNext())
		{
			...
			System.out.println("If the win amount is 51.25, gamble count should be 1.");
			finder14.destroy();  
		}         
		else    
		{ 
		...
		if(finder15.hasNext())
		{
			...
			System.out.println("If the win amount is 60, gamble should be disabled");
			finder15.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed on gamble page of Brownie Hot slot game based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Brownie Hot game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_Brownie_Hot_slot_game_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_in_Brownie_Hot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
