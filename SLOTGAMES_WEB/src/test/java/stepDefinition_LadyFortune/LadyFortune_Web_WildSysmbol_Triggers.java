...
public class LadyFortune_Web_WildSysmbol_Triggers {
	...
	public void chrome_browser_valid_URL_valid_login_details_Lady_Fortune_slot_game_bet_type_denomination_balance_wild_symbols_all_symbols_spin_button_Big_win_window_and_win_amount() throws Throwable {

		this.driver = LadyFortune_URL_Login.getDriver();
	}

	@When("^Open the Lady Fortune slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, select bet type & denomination, click on spin button till wild symbols appears and substitues for all other symbols expect scatter symbol and  triggers winamt\\*(\\d+) and on appear of (\\d+) wild symbols on all reels big win is triggered and win amount added to balance$")
	public void open_the_Lady_Fortune_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_select_bet_type_denomination_click_on_spin_button_till_wild_symbols_appears_and_substitues_for_all_other_symbols_expect_scatter_symbol_and_triggers_winamt_and_on_appear_of_wild_symbols_on_all_reels_big_win_is_triggered_and_win_amount_added_to_balance(int arg1, int arg2) throws Throwable {
		...
		// win amount
		Pattern winamt1=new Pattern("./Images/Lady_Fortune/wildwin_2.6.PNG");
		Pattern winamt2=new Pattern("./Images/Lady_Fortune/wildwin_53.8.PNG");
		Pattern bigwin=new Pattern("./Images/Lady_Fortune/wildBigwin_203.8.PNG");
		...
		if(finder12.hasNext())
		{
			...
			System.out.println("Comparision happened successfully and BigWin amount is ");
			finder12.destroy();  
		}         
		else    
		{ 
	...
	@Then("^wild win amount with other symbol triggers win amount\\*(\\d+) and upon appearing of (\\d+) wild symbols Big Win screen should appear when five wild symbols appears on the screen in Lady Fortune game$")
	public void wild_win_amount_with_other_symbol_triggers_win_amount_and_upon_appearing_of_wild_symbols_Big_Win_screen_should_appear_when_five_wild_symbols_appears_on_the_screen_in_Lady_Fortune_game(int arg1, int arg2) throws Throwable {
