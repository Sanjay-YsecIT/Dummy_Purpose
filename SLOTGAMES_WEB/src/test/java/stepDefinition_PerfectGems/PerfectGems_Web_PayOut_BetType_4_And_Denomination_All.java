...
public class PerfectGems_Web_PayOut_BetType_4_And_Denomination_All {
	WebDriver driver;
	Screen screen=new Screen();

	@Given("^Chrome browser, valid URL, valid login details, Perfect Gems slot game, icon to open payout table, all symbols, max amount on each symbol like 'K' ,'A' n etc, back button, bet type (\\d+)\\.(\\d+) and denomination values like (\\d+), (\\d+), (\\d+) & (\\d+) on web$")
	public void chrome_browser_valid_URL_valid_login_details_Perfect_Gems_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_K_A_n_etc_back_button_bet_type_and_denomination_values_like_on_web(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6) throws Throwable {
		this.driver = PerfectGems_URL_Login.getDriver();
	}

	@When("^Open the Perfect Gems slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like 'K' ,'A' n etc, click on back button, select bet type as (\\d+)\\.(\\d+) & denomination  values like (\\d+), (\\d+), (\\d+) & (\\d+) and check payout amount on web$")
	public void open_the_Perfect_Gems_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_K_A_n_etc_click_on_back_button_select_bet_type_as_denomination_values_like_and_check_payout_amount_on_web(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6) throws Throwable {
		...
		Pattern pat1=new Pattern("Images/Perfect_Gems/payout4_1.PNG");
		Pattern pat2=new Pattern("Images/Perfect_Gems/payout4_2.PNG");
		Pattern pat3=new Pattern("Images/Perfect_Gems/payout4_3.PNG");
		Pattern pat4=new Pattern("Images/Perfect_Gems/payout4_4.PNG");
		Pattern pat11=new Pattern("Images/Perfect_Gems/payout4_11.PNG");
		Pattern pat22=new Pattern("Images/Perfect_Gems/payout4_22.PNG");
		Pattern pat33=new Pattern("Images/Perfect_Gems/payout4_33.PNG");
		Pattern pat44=new Pattern("Images/Perfect_Gems/payout4_44.PNG");
		...
		System.out.println("Comparing the max payout value for 30 denomination and comparision value equals to: "+" "+score33 +"%");
		...
		System.out.println("Comparing the max payout value for 40 denomination and comparision value equals to: "+" "+score4 +"%");
		...
		String ht51 = finder51.find(pat22);
		double score51=20;                
		...
		System.out.println("Comparing the max payout value for 20 denomination and comparision value equals to: "+" "+score51 +"%");
		Assert.assertTrue(score51 > 97);
		Thread.sleep(2000);
	}

	@Then("^System should display the Payout max amount on each symbol like 'K' ,'A' n etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination values like (\\d+), (\\d+), (\\d+) & (\\d+) in Perfect Gems slot game on web$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_K_A_n_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_values_like_in_Perfect_Gems_slot_game_on_web(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6) throws Throwable {
		Thread.sleep(2000);
		driver.quit();
	}
}
