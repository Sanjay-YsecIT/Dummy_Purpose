...
public class PerfectGems_Web_Check_BetType_BetValue_DropDown3 {
	WebDriver driver;
	...
	public void open_the_Perfect_Gems_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_from_down_and_check_the_bet_value_should_be(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9) throws Throwable {
		...
		Pattern bet=new Pattern("Images/Perfect_Gems/bet3.PNG");
		Pattern bet_dropdown3=new Pattern("Images/Perfect_Gems/bet_dropdown3.PNG");
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("Comparision happened successfully. Available denominations are: 1.25,2.5,3.75,5. Test case passed");
			finder1.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Bet value should get changed to (\\d+)\\.(\\d+),(\\d+)\\.(\\d+),(\\d+)\\.(\\d+),(\\d+) if the bet type selected as (\\d+)\\.(\\d+) from the drop down three in Perfect Gems slot game$")
	public void bet_value_should_get_changed_to_if_the_bet_type_selected_as_from_the_drop_down_three_in_Perfect_Gems_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9) throws Throwable {
		Thread.sleep(2000);
		driver.quit();
	}
}
