...
public class PerfectGems_Web_TryNow_Spin_Deduct_Balance {
	WebDriver driver;
	...
	public void chrome_browser_valid_URL_valid_login_details_Perfect_Gems_slot_game_try_now_button_balance_to_play_and_spin_button() throws Throwable {
	...
	public void open_the_Perfect_Gems_slot_game_by_entering_the_valid_URL_in_browser_click_on_try_now_button_and_click_on_spin_button() throws Throwable {
	
		Pattern TryNow_bal=new Pattern("Images/Perfect_Gems/TryNow_balance.PNG");
		Pattern BalAfterSpin=new Pattern("Images/Perfect_Gems/TN_balance1_1.PNG");
		Pattern spin=new Pattern("Images/Perfect_Gems/TryNow_spin.PNG");
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("Balance should be 4999.75 in try now screen and comparision successfull");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		System.out.println("Balance should be 4999.75 in try now screen and comparision value equals to: "+" "+score1 +"%");
		Assert.assertTrue(score1 > 97);
	}

	@Then("^Amount should get deducted from balance on click on spin button in try now page of Perfect Gems slot game$")
	public void amount_should_get_deducted_from_balance_on_click_on_spin_button_in_try_now_page_of_Perfect_Gems_slot_game() throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
