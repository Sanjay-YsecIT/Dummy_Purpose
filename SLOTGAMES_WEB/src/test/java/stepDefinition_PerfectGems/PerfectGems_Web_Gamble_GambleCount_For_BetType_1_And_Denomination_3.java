...
public class PerfectGems_Web_Gamble_GambleCount_For_BetType_1_And_Denomination_3 {
	WebDriver driver;
	Screen screen=new Screen();
	
	@Given("^Perfect Gems slot game, bet type as (\\d+)\\.(\\d+), denomination as (\\d+)\\.(\\d+), balance, spin button, win amount, gamble button, gamble amount, game info page and gamble count in gamble page of slot game$")
	public void perfect_Gems_slot_game_bet_type_as_denomination_as_balance_spin_button_win_amount_gamble_button_gamble_amount_game_info_page_and_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		this.driver = PerfectGems_URL_Login.getDriver();
	}

	@When("^Open the Perfect Gems slot game by entering the valid URL in browser, enter the valid login details, enter the amount, transfer the balance, click on Perfect Gems slot game, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count in gamble page of slot game$")
	public void open_the_Perfect_Gems_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_enter_the_amount_transfer_the_balance_click_on_Perfect_Gems_slot_game_select_bet_type_as_denomination_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count_in_gamble_page_of_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		// Gamble win amount
		Pattern win_amt1=new Pattern("Images/Perfect_Gems/win_g_3_1.PNG");  
		Pattern win_amt2=new Pattern("Images/Perfect_Gems/win_g_3_2.PNG");   
		Pattern win_amt3=new Pattern("Images/Perfect_Gems/win_g_3_3.PNG");    
		Pattern win_amt4=new Pattern("Images/Perfect_Gems/win_g_3_4.PNG");    
		Pattern win_amt5=new Pattern("Images/Perfect_Gems/win_g_3_5.PNG");    
		Pattern win_amt6=new Pattern("Images/Perfect_Gems/win_g_3_6.PNG");    
		...
		if(finder1.hasNext())
		{
			...
			System.out.println("If the win amount is 1.20, gamble count should be 5.");
			finder1.destroy();  
		}         
		else    
		{ 
		...
		if(finder13.hasNext())
		{
			...
			System.out.println("If the win amount is 9.60, gamble count should be 2.");
			finder13.destroy();  
		}         
		else    
		{ 
		...
		if(finder14.hasNext())
		{
			...
			System.out.println("If the win amount is 19.20, gamble count should be 1.");
			finder14.destroy();  
		}         
		else    
		{ 
		...
		//Arranging the symbols to generate win amount greater than set value
		...
		if(finder15.hasNext())
		{
			...
			System.out.println("If the win amount is 28.80, gamble should be disabled");
			finder15.destroy();  
		}         
		else    
		{ 
	...
	@Then("^Gamble count should be displayed on gamble page of Perfect Gems slot game based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Perfect Gems slot game$")
	public void gamble_count_should_be_displayed_on_gamble_page_of_Perfect_Gems_slot_game_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_in_Perfect_Gems_slot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		Thread.sleep(2000);
		driver.quit();
	}
}
